	...
	public void memberMainUi(ModongUser user) {
		...
		while (true) {
			...
			case 0:
				...
				break;
			case 2:
				System.out.println("point를 사용 합니다. 잔여 포인트 : " + user.getUser_point());
			...
			case 3:
				System.out.println("point를 기부 합니다. 잔여 포인트 : " + user.getUser_point());
				System.out.println("기부 id를 입력해 주세요.");
				int did = scan.nextInt();
				System.out.println("사용할 포안트를 입력해 주세요.");
				int point3 = scan.nextInt();
				if (user.removePoint(point3))
					doa.recordDonationPoint(user.getUser_bacode(), did, point3);
				break;

			case 1:
				System.out.println("point를 적립 합니다. 잔여 포인트 : " + user.getUser_point()); // 월래는
																						// cid
																						// or
																						// pid필요함
